...
export default function CourseDetailModal({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-[#0b1120]/90 backdrop-blur-md flex justify-center items-start p-2 sm:p-4 md:p-6 select-none animate-fadeIn">
      ...
      {/* Modal Container */}
      <div className="relative w-full max-w-4xl bg-[#0f172a] border border-slate-800 rounded-3xl overflow-hidden shadow-2xl my-4 sm:my-8 text-white">

        {/* Floating Close Button */}
        <button
          onClick={onClose}
          className="absolute top-4 right-4 z-40 p-2.5 rounded-full bg-slate-900/80 border border-slate-700 text-white hover:bg-slate-800 transition-all cursor-pointer shadow-lg"
        ...
        <div className="relative h-64 sm:h-80 md:h-96 w-full overflow-hidden bg-[#0b1120]">
          <img
            src={course.backdrop}
            alt={course.title}
          ...
          <div className="absolute inset-0 bg-gradient-to-r from-[#0f172a] via-[#0f172a]/70 to-transparent w-full sm:w-2/3" />

          {/* Banner Meta Content */}
          <div className="absolute bottom-6 left-6 right-6 z-20 space-y-3">
            {course.badge && (
              <span className="inline-flex items-center gap-1.5 bg-sky-600/90 text-white font-black text-[10px] sm:text-xs px-2.5 py-1 rounded uppercase tracking-wider shadow">
            ...
            <p className="text-xs sm:text-sm font-medium text-slate-300 max-w-xl line-clamp-2">
              {course.subtitle}
            </p>
            ...
            <div className="flex flex-wrap items-center gap-3 pt-2">
              <button
                ...
                className="flex items-center gap-2 bg-sky-500 hover:bg-sky-400 text-white font-black text-xs sm:text-sm px-5 py-2.5 rounded-xl transition-all cursor-pointer shadow-xl active:scale-95"
              >
              ...
              <button
                onClick={handleShare}
                className="p-2.5 rounded-xl bg-slate-900/80 hover:bg-slate-800 border border-slate-700 text-slate-300 hover:text-white cursor-pointer relative"
                title="Share Course"
              >
                <Share2 size={16} />
                {copiedShare && (
                  <span className="absolute -top-8 left-1/2 -translate-x-1/2 bg-slate-900 text-white text-[10px] font-bold px-2 py-0.5 rounded shadow border border-slate-700">
                    Copied!
                  </span>
                )}
              </button>
        ...
        {/* Course Stats Row */}
        <div className="px-6 py-4 bg-[#0b1120] border-y border-slate-800 flex flex-wrap items-center justify-between gap-4 text-xs font-semibold text-slate-300">
          <div className="flex items-center gap-3">
            <span className="text-sky-400 font-bold">{course.matchScore}% Match</span>
            <span className="text-amber-400 font-bold flex items-center gap-1">
              <Star size={13} className="fill-amber-400" />
              <span>{course.rating}</span>
            </span>
            <span className="px-2 py-0.5 rounded border border-slate-700 bg-slate-900 text-slate-200">
              {course.level}
            </span>
          ...
          >
            ...
            />
            <span className="text-slate-300 group-hover:text-sky-400 font-bold">
        ...
        <div className="px-6 border-b border-slate-800 flex gap-6 text-xs sm:text-sm font-bold text-slate-400 pt-3">
          <button
        ...
        <div className="p-6 space-y-6">
              ...
              <div className="space-y-3">
                {course.lessons.map((lesson) => (
                  <div
                    ...
                    className="p-4 rounded-2xl bg-slate-900/80 hover:bg-slate-800/90 border border-slate-800/80 transition-all cursor-pointer flex items-center justify-between gap-4 group"
                  >
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 rounded-full bg-sky-500/20 text-sky-400 group-hover:bg-sky-500 group-hover:text-white flex items-center justify-center shrink-0 transition-colors">
                      ...
                      <div className="space-y-1">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs text-slate-400 font-bold">&bull; {lesson.duration}</span>
                        </div>
                    ...
                    <div className="text-xs font-bold text-slate-400 group-hover:text-white shrink-0">
                      Watch &rsaquo;
                    </div>
                  </div>
                ))}
              ...
              <div className="space-y-3 pt-2">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  {course.whatYouWillLearn.map((item, idx) => (
                    <div
                      key={idx}
                      className="p-3.5 bg-slate-900 border border-slate-800 rounded-xl text-xs font-semibold text-slate-300 flex items-start gap-2.5"
                    >
              ...
              <div className="p-4 bg-slate-900 border border-slate-800 rounded-2xl flex items-center justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-slate-400 font-medium">{teacher.title}</p>
                  </div>
                </div>
          ...
          {activeTab === "localization" && (
            <div className="space-y-6">
              <div className="p-4 bg-slate-900 border border-sky-800/40 rounded-2xl space-y-2">
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {course.availableLanguages.map((lang) => (
                      <div
                        key={lang}
                        className="p-3 bg-slate-900 border border-slate-800 rounded-xl text-xs font-bold flex items-center justify-between"
                      >
                ...
                <div className="space-y-3">
                  ...
                  <div className="space-y-2">
                    {["Off", "Deutsch", "English", "Français", "Español", "Kiswahili"].map((sub) => (
                      <div
                        key={sub}
                        className="p-3 bg-slate-900 border border-slate-800 rounded-xl text-xs font-bold flex items-center justify-between text-slate-300"
                      >
                        <span>{sub}</span>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
              {relatedCourses.map((rel) => (
                <div
                  ...
                  className="p-3 bg-slate-900 border border-slate-800 rounded-2xl hover:border-slate-600 transition-all cursor-pointer space-y-2"
                >
                  <img
                    src={rel.thumbnail}
                    alt={rel.title}
                  ...
                  <p className="text-[11px] text-slate-400">{rel.duration} &bull; {rel.level}</p>
                </div>
              ))}
            </div>
          )}
