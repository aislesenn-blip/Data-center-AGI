...
import { X, Star, Users, Clock, Globe, Play } from "lucide-react";
...
export default function TeacherProfileModal({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-black/90 backdrop-blur-md flex justify-center items-start p-2 sm:p-4 md:p-6 select-none animate-fadeIn">
      <div className="relative w-full max-w-3xl bg-[#181818] border border-zinc-800 rounded-3xl p-6 sm:p-8 space-y-6 shadow-2xl my-6 text-white">
        ...
        <div className="flex flex-col sm:flex-row items-center sm:items-start gap-5 pt-2">
          <img
            src={teacher.avatar}
            alt={teacher.name}
            className="w-24 h-24 sm:w-28 sm:h-28 rounded-full object-cover border-4 border-red-600/80 shadow-2xl shrink-0"
          />

          <div className="space-y-2 text-center sm:text-left">
            <div className="flex flex-wrap items-center justify-center sm:justify-start gap-2">
              <h1 className="text-2xl font-heading font-black text-white">{teacher.name}</h1>
              <span className="bg-red-600/20 text-red-400 border border-red-500/30 text-[10px] font-black px-2 py-0.5 rounded-full uppercase">
                Featured Teacher
              </span>
            </div>

            <p className="text-xs font-bold text-zinc-400">{teacher.title}</p>
            <p className="text-xs text-zinc-300 leading-relaxed max-w-lg">{teacher.bio}</p>

            <div className="text-[11px] font-bold text-zinc-400 flex items-center justify-center sm:justify-start gap-1 pt-1">
        ...
        {/* Stats Row */}
        <div className="grid grid-cols-3 gap-3 p-4 bg-zinc-900 border border-zinc-800 rounded-2xl text-center">
          <div>
            <div className="text-lg font-black text-white">{teacher.learnersCount.toLocaleString()}</div>
            <p className="text-[10px] font-bold text-zinc-400 uppercase">Learners</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="text-[10px] font-bold text-zinc-400 uppercase">Watch Time</p>
          </div>
        </div>
